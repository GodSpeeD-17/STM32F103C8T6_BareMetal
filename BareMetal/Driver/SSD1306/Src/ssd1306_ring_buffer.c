...
void SSD1306_RB_Disp_Init(ssd1306_config_t* ssd1306){
	...
	// Initialize the Cursor Position
	Ring_Buffer_Enqueue(&ssd1306->i2c_rb, SSD1306_CMD_PAGE_MODE_SET_X_LOWER_NIBBLE(ssd1306->cursor.X));
	Ring_Buffer_Enqueue(&ssd1306->i2c_rb, SSD1306_CMD_PAGE_MODE_SET_X_UPPER_NIBBLE(ssd1306->cursor.X));
	Ring_Buffer_Enqueue(&ssd1306->i2c_rb, SSD1306_CMD_PAGE_MODE_SET_Y(ssd1306->cursor.Y));
	// Update the Cursor Position in the Display Buffer
	SSD1306_RB_Disp_Cursor_Update(ssd1306);
...
void SSD1306_RB_Goto_XY(ssd1306_config_t* ssd1306, const uint8_t X, const uint8_t Y){
	...
	ssd1306->cursor.X = (X & 0x7F);
	ssd1306->cursor.Y = (Y & 0x3F);
	...
	I2C_Master_Write_Start(ssd1306->i2c_config.I2Cx, ssd1306->address, SSD1306_CMD_INDICATOR);
	// <<< Trigger I2C Data Transfer from Ring Buffer>>>
	I2C_Ring_Buffer_TX_Block(ssd1306->i2c_config.I2Cx, &ssd1306->i2c_rb, 3);
	// <<< I2C Master Stop Sequence >>>
	I2C_Master_Stop(ssd1306->i2c_config.I2Cx);
}
...
uint8_t SSD1306_RB_Set_Page_Pattern(ssd1306_config_t* ssd1306, uint8_t page, const uint8_t pattern){
	// Page Out of Bound
	if(page >= SSD1306_PAGE){
		// Wrap the Page Number
		page &= 0x07;
	}
	// Update the Cursor Position
	SSD1306_RB_Goto_XY(ssd1306, ((page & 0x07) << 3), 0);
	...
	for(uint8_t index = 0; index < SSD1306_WIDTH; index++){
		...
		if(!Ring_Buffer_Enqueue(&ssd1306->i2c_rb, pattern)){
	...
	if(I2C_Ring_Buffer_TX_Block(ssd1306->i2c_config.I2Cx, &ssd1306->i2c_rb, SSD1306_WIDTH)){
		...
		// Update the Cursor Position
		SSD1306_RB_Goto_XY(ssd1306, (page << 3), (SSD1306_WIDTH - 1));
	}
	// <<< I2C Master Stop Sequence >>>
	I2C_Master_Stop(ssd1306->i2c_config.I2Cx);
	// Go to next Page
	SSD1306_RB_Goto_XY(ssd1306, ((page + 1) << 3), 0);
	// Success
	return 0x01;
}
...
uint8_t SSD1306_RB_Set_Disp_Pattern(ssd1306_config_t* ssd1306, const uint8_t pattern){
	...
	volatile uint32_t delay = 0x000FFFFF;
