...
void ADC_config(adc_config_t* ADC_CONFIGx){
	...
	GPIO_Init
	(
		ADC_CONFIGx->GPIOx_CONFIG->GPIO,
		ADC_CONFIGx->GPIOx_CONFIG->PIN,
		ADC_CONFIGx->GPIOx_CONFIG->MODE,
		ADC_CONFIGx->GPIOx_CONFIG->CNF
	);
	...
	if(ADC_CONFIGx->enable_IRQ == (ADCx_IRQ_ENABLE)){
		...
		// Enable NVIC IRQ
		NVIC_IRQEnable(ADC_get_IRQn(ADC_CONFIGx));
	}
	// Starts the ADC
