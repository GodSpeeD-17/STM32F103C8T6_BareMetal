 ...
 * application service enabled by `app_config.h`.
 *
 * @section APP_INIT_H_BOUNDARY Dependency Boundary
 * This header includes shared status types only. RCC, SysTick, Timer, and
 * application-service headers remain private to `app_init.c` and do not leak
 * upward into startup.
 */
 ...
 * when enabled, then initializes the optional dedicated microsecond-delay
 * Timer. Processing stops at the first failed transaction.
